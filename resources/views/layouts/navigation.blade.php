            ...
            <div class="flex items-stretch justify-end h-full ml-auto">
                @if (Auth::check())
                    ...
                    <x-dropdown contentClasses="p-1 bg-white fmr-2">
                        <x-slot name="trigger">
                            <img src="{{ isset(Auth::user()->profile_image) ? asset(Auth::user()->profile_image) : asset('images/user_icon.png') }}" class="w-12 h-12 m-2 bg-gray-200 rounded-full cursor-pointer active:scale-90 hover:scale-95" alt="photo">
                        </x-slot>
                        <x-slot name="content">
